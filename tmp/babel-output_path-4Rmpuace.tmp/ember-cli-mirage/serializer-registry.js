  ...
  var SerializerRegistry = (function () {
    ...
    _createClass(SerializerRegistry, [{
      ...
      value: function serialize(response, request) {
        ...
        } else if ((0, _lodashLangIsArray['default'])(response) && response.filter(this._isCollection).length) {
          return response.reduce(function (json, collection) {
            var serializer = _this.serializerFor(collection.modelName);

            if (serializer.embed) {
              json[(0, _emberCliMirageUtilsInflector.pluralize)(collection.modelName)] = serializer._serializeModelOrCollection(collection, request);
            } else {
              json = (0, _lodashObjectAssign['default'])(json, serializer._serializeSideloadedModelOrCollection(collection, request));
            }
      ...
      value: function serializerFor(type) {
        ...
        var _ref$alreadySerialized = _ref.alreadySerialized;
        var alreadySerialized = _ref$alreadySerialized === undefined ? {} : _ref$alreadySerialized;
